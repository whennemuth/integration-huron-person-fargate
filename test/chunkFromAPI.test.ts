...
describe('ChunkFromAPI Parameter Gathering', () => {
  ...
  describe('Parameter gathering from environment variables', () => {
    it('should read task parameters from environment when all are provided', () => {
      process.env.BASE_URL = 'https://api.env.com';
      process.env.FETCH_PATH = '/env/people';
    ...
    it('should handle missing FETCH_PATH', () => {
      process.env.BASE_URL = 'https://api.env.com';
      delete process.env.FETCH_PATH;
      process.env.POPULATION_TYPE = 'person-full';
    ...
    it('should handle missing both BASE_URL and FETCH_PATH', () => {
      delete process.env.BASE_URL;
      delete process.env.FETCH_PATH;
      process.env.POPULATION_TYPE = 'person-full';
  ...
  describe('Parameter gathering from SQS message', () => {
    ...
    it('should set task parameters from queue message with all fields', () => {
      const messageBody = {
        BASE_URL: 'https://api.queue.com',
        FETCH_PATH: '/queue/people',
    ...
    it('should handle from_config placeholder for BASE_URL', () => {
      const messageBody = {
    ...
    it('should handle from_config placeholder for FETCH_PATH', () => {
      const messageBody = {
        BASE_URL: 'https://api.queue.com',
        FETCH_PATH: 'from_config',
        POPULATION_TYPE: 'person-full'
      };
  ...
  describe('Parameter priority and combination scenarios', () => {
    ...
    it('should merge environment BASE_URL with config FETCH_PATH', () => {
      process.env.BASE_URL = 'https://api.env.com';
      delete process.env.FETCH_PATH;
      process.env.POPULATION_TYPE = 'person-full';

      const chunker = new ChunkFromAPI(mockConfig);
      // Should have BASE_URL from env, FETCH_PATH will come from config in hasSufficientConfig
      expect(chunker.hasSufficientConfig()).toBe(true);
    });

    it('should merge config BASE_URL with environment FETCH_PATH', () => {
      delete process.env.BASE_URL;
      process.env.FETCH_PATH = '/env/people';
      process.env.POPULATION_TYPE = 'person-full';

      const chunker = new ChunkFromAPI(mockConfig);
      // Should have FETCH_PATH from env, BASE_URL will come from config in hasSufficientConfig
      expect(chunker.hasSufficientConfig()).toBe(true);
    });
    ...
    it('should handle partial message with config fallback', () => {
      ...
      const messageBody = {
        BASE_URL: 'https://api.queue.com',
        // FETCH_PATH missing, should fall back to config
        POPULATION_TYPE: 'person-full'
      };
